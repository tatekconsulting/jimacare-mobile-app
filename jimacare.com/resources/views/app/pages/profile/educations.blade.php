		...
		<div>
			@foreach($educations as $education)
				<label for="education_{{ $education->id }}"
					   ...
					   @if(old('education',in_array($education->id, $profile->educations->pluck('id')->toArray()))) aria-pressed="true"
				...
				>
					<input type="checkbox" name="education[]" value="{{ $education->id }}"
						   ...
						   @if(old('education',in_array($education->id, $profile->educations->pluck('id')->toArray()))) checked @endif
						   @if( count($profile->educations->pluck('id')) < 1 ) required @endif
					/> {{ $education->title }}
				</label>
			@endforeach
		</div>

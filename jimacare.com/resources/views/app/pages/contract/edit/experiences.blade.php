@if(($role->id ?? 0) > 2)
        <div class="form-group col-12 cb-must-select">
                ...
                <div>
                        @php $exps = $contract->experiences ? $contract->experiences->pluck('id')->toArray() : []; @endphp
                        @foreach($experiences ?? [] as $exp)
                                <label for="experience_{{ $exp->id }}" class="btn btn-outline-primary"
                                           data-toggle="button"
                                           @if(in_array($exp->id, $exps )) aria-pressed="true"
                                           @else aria-pressed="false" @endif
                                >
                                        <input type="checkbox" name="experience[]" value="{{ $exp->id }}"
                                                   id="experience_{{ $exp->id }}" class="experience"
                                                   @if(in_array($exp->id, $exps )) checked @endif
                                                   @if( count($exps) < 1 ) required @endif
                                        />{{ $exp->title }}
                                </label>
                        @endforeach
                </div>

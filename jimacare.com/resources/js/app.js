...
// Register Vue Components for new features
Vue.component('available-now-toggle', require('./components/AvailableNowToggle.vue').default);
Vue.component('push-notification-toggle', require('./components/PushNotificationToggle.vue').default);
Vue.component('chatbot-widget', require('./components/ChatbotWidget.vue').default);
...
jQuery(document).ready(function ($) {
	...
	body.on('change', '.pay_rate input', function (e) {
		var required = true, inputs = $('.pay_rate input');
		...
		if (required) {
			$('.pay_rate input').attr('required', true);
		} else {
			$('.pay_rate input').removeAttr('required');
		}
			...
			$.ajax({
				...
				success: function (res) {
					...
					var temp = "" +
						...
						"       <img src='" + (res['profile'] || '/img/undraw_profile.svg') +  "' alt='' class='img-avatar'>" +
						"   </div>" +
						"   <div class='media-body'>" +
	...
	// Removed: Interested/Not Interested buttons (replaced with Accept/Reject)
...
// Initialize Vue app for new components
